...
class ConnectionManager {
	...
	rejectConnection(ws, reason, code) {
		log.warn(`WebSocket connection rejected - Reason: ${reason}, Code: ${code}`);
		...
		setTimeout(() => {
			try {
				...
				log.error('Error closing rejected connection:', error);
			}
		}, 100);
	}
	...
	setupConnectionHandlers(ws) {
		...
		log.proxy(`Connection event handlers setup - ConnectionID: ${ws.connectionId}`);
	}
	...
	handleConnectionError(ws, error) {
		log.error(`WebSocket connection error - ConnectionID: ${ws.connectionId}, Error: ${error.message}`);
